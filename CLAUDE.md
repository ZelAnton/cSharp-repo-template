...
**Step ordering is built around one invariant: the NuGet publish is the single irreversible step, so it is the pivot.** A required-secret preflight runs first (fails fast if `NUGET_API_KEY` is missing, before anything external happens). Everything before the pivot is cheap and reversible — build, test, pack, and a *local* commit + tag that is not yet pushed. The publish runs next (retried for transient failures; `--skip-duplicate` lets an in-step retry recognize an accepted version). Only after it succeeds does the workflow push the commit + tag to `main` (atomically) and create the GitHub Release (idempotent: edits an existing Release, else creates one; retried). Re-run only when NuGet did not accept the package. **Once NuGet accepts it, never re-run or rebuild the release, regardless of whether the tag push succeeded.** Download that run's immutable `release-recovery-vX.Y.Z` artifact and use its exact bundle, packages, checksums, and release notes to finish the tag push or GitHub Release.
