...
**Step ordering is built around one invariant: the NuGet publish is the single irreversible step, so it is the pivot.** A required-secret preflight runs first (fails fast if `NUGET_API_KEY` is missing, before anything external happens). Everything before the pivot is cheap and reversible — build, test, pack, and a *local* commit + tag that is not yet pushed. The publish runs next (retried for transient failures; `--skip-duplicate` treats "this version already exists" as success). Only after it succeeds does the workflow push the commit + tag to `main` (atomically) and create the GitHub Release (idempotent: edits an existing Release, else creates one; retried). Consequences for recovery: a failure **before or at** the publish leaves no trace on the remote or registry — just re-run. A failure **after** the publish but before the tag is pushed is also safe to re-run (same version recomputed, republish is a no-op). But **once the tag is on `main`, do not blindly re-run the whole workflow** — a re-run computes the *next* version and orphans the release you just published; finish the GitHub Release by hand instead (the failing step prints the exact `gh release create` command).
