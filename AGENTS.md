...
- **Step ordering invariant:** the NuGet publish is the only irreversible step, so it is the pivot. A secret preflight runs first; build/test/pack and a *local* (unpushed) commit+tag run before the pivot; the publish (retried, `--skip-duplicate`) runs next; the atomic commit+tag push to `main` and the idempotent+retried GitHub Release run after. A failure before or at the publish leaves no remote/registry trace — re-run freely. **Once the tag is pushed to `main`, do not re-run the whole workflow** (it would bump to the next version and orphan the published release) — finish the GitHub Release manually using the command the failing step prints. When editing `release.yml`, preserve this ordering: never move the publish after the tag push, and keep the post-pivot steps idempotent.
